...
// PADRÃO MVC (model view and controller) - separa a lógica de negócios da apresentação
// Controller faz a ligação entre o model e a view 
// O controller vai lidar com as solicitações do usuário por recursos dos servidor
...
exports.find = (req, res) => {
    ...
    if (req.query.id) {
        ...
        Userdb.findById(id)
            .then(data => {
                if (!data) {
                    res.status(404).send({ message: "Usuário com id " + id + " não foi encontrado!" })
                } else {
                    res.send(data)
                }
            })
...
exports.update = (req, res) => {
    ...
    Userdb.findByIdAndUpdate(id, req.body, { useFindAndModify: false })
        .then(data => {
            if (!data) {
                res.status(404).send({ message: `Não é possível atualizar o usuário com ${id}. Talvez o usuário não foi encontrado!` })
            } else {
                res.send(data)
            }
        })
